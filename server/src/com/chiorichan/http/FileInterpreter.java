...
public class FileInterpreter
{
	...
	public FileInterpreter( HttpRequest request ) throws IOException, HttpErrorException
	{
		...
		try
		{
			...
			if ( sql.getRowCount( rs ) > 0 )
			{
				...
				if ( rewrite.size() > 0 )
				{
					...
					Loader.getLogger().warning( "Failed to find a page redirect for Rewrite... '" + subdomain + "." + domain + "' '" + uri + "'" );
			}
			else
				Loader.getLogger().warning( "Failed to find a page redirect for Rewrite... '" + subdomain + "." + domain + "' '" + uri + "'" );
		}
		...
		if ( wasSuccessful )
		{
			Loader.getLogger().info( "Interpreted page request to " + interpParams );
