 ...
 */
public abstract class GroovyPlugin extends PluginBase
{
	...
	public GroovyPlugin()
	{
		...
		if ( !( classLoader instanceof GroovyPluginClassLoader ) )
		{
			throw new IllegalStateException( "GroovyPlugin requires " + GroovyPluginClassLoader.class.getName() );
		}
		( (GroovyPluginClassLoader) classLoader ).initialize( this );
	}
	
	protected GroovyPlugin(final GroovyPluginLoader loader, final PluginDescriptionFile description, final File dataFolder, final File file)
	{
		final ClassLoader classLoader = this.getClass().getClassLoader();
		if ( classLoader instanceof GroovyPluginClassLoader )
		{
	 ...
	 * @throws IllegalArgumentException if clazz does not extend {@link GroovyPlugin}
	 * @throws IllegalStateException if clazz was not provided by a plugin,
	 *              for example, if called with <code>GroovyPlugin.getPlugin(GroovyPlugin.class)</code>
	 * @throws IllegalStateException if called from the static initializer for
	 *              given GroovyPlugin
	 * @throws ClassCastException if plugin that provided the class does not
	 *              extend the class
	 */
	public static <T extends GroovyPlugin> T getPlugin( Class<T> clazz )
	{
		Validate.notNull( clazz, "Null class cannot have a plugin" );
		if ( !GroovyPlugin.class.isAssignableFrom( clazz ) )
		{
			throw new IllegalArgumentException( clazz + " does not extend " + GroovyPlugin.class );
		}
		final ClassLoader cl = clazz.getClassLoader();
		if ( !( cl instanceof GroovyPluginClassLoader ) )
		{
			throw new IllegalArgumentException( clazz + " is not initialized by " + GroovyPluginClassLoader.class );
		}
		GroovyPlugin plugin = ( (GroovyPluginClassLoader) cl ).plugin;
		if ( plugin == null )
		{
	 ...
	 *              given GroovyPlugin
	 */
	public static GroovyPlugin getProvidingPlugin( Class<?> clazz )
	{
		Validate.notNull( clazz, "Null class cannot have a plugin" );
		final ClassLoader cl = clazz.getClassLoader();
		if ( !( cl instanceof GroovyPluginClassLoader ) )
		{
			throw new IllegalArgumentException( clazz + " is not provided by " + GroovyPluginClassLoader.class );
		}
		GroovyPlugin plugin = ( (GroovyPluginClassLoader) cl ).plugin;
		if ( plugin == null )
		{
