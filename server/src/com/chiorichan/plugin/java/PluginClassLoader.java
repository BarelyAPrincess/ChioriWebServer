...
public class PluginClassLoader extends URLClassLoader
{
	...
	{
		return extended ? findClass( name, true ) : findClass0( name, true ); // Don't warn on deprecation, but maintain
																										// overridability
	}
	...
	Class<?> findClass0( String name, boolean checkGlobal ) throws ClassNotFoundException
	{
		...
		if ( result == null )
		{
			if ( checkGlobal )
			{
				result = loader.extended ? loader.getClassByName( name ) : loader.getClassByName0( name ); // Don't warn on
																																			// deprecation,
																																			// but maintain
																																			// overridability
			}
			...
			if ( result == null )
			{
				...
				{
					if ( loader.extended )
					{ // Don't warn on deprecation, but maintain overridability
