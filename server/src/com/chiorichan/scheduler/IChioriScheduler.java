...
public interface IChioriScheduler
{
	 ...
	 * thread.
	 * <p>
	 * Note: The Future.get() methods must NOT be called from the main thread. Note2: There is at least an average of
	 * 10ms latency until the isDone() method returns true.
	 * 
	 * @param <T>
	 ...
	 * <p>
	 * A repeating task might not be running currently, but will be running in the future. A task that has finished, and
	 * does not repeat, will not be running ever again.
	 * <p>
	 ...
	 * <p>
	 * If a repeating task is currently running, it might not be queued now but could be in the future. A task that is
	 * not queued, and not running, will not be queued again.
	 * 
	 * @param taskId
	 *           The task to check.
	 *           <p>
