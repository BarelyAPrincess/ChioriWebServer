...
public class HttpsManager implements ServerManager
{
	...
	public void addMapping( String hostname, File sslCert, File sslKey, String sslSecret ) throws SSLException, FileNotFoundException
	{
		...
		NetworkManager.getLogger().info( String.format( "Initalizing a new SslContext using cert '%s', key '%s', and hasSecret? %s for hostname ''", sslCert.getName(), sslKey.getName(), ( sslSecret != null && !sslSecret.isEmpty() ), hostname ) );
	 ...
	 * Warning: if you are updating the certificate the previous virtual host mappings will be lost to the GC
	 * 
	 * @param sslCert
	...
	public void updateCertificate( final File sslCert, final File sslKey, final String sslSecret ) throws FileNotFoundException, SSLException
	{
		...
		for ( Site s : SiteManager.INSTANCE.getSites() )
		{
