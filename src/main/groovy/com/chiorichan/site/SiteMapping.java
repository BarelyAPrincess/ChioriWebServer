...
public class SiteMapping
{
	...
	public static void put( String domain, Site site )
	{
		...
		if ( currentNode.getMapping() != null )
			throw new IllegalStateException( String.format( "Can't override the site '%s' with site '%s' for domain '%s', each site must have a unique domain, subdomains included.", currentNode.getMapping().getSite().getSiteId(), site.getSiteId(), domain ) );
