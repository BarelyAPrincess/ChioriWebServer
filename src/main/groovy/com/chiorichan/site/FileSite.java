...
public class FileSite implements Site
{
	...
	FileSite( YamlConfiguration yaml ) throws SiteException
	{
		...
		if ( yaml.isList( "subdomains" ) )
		{
			List<String> subdomains = yaml.getStringList( "subdomains" );
			ConfigurationSection section = yaml.createSection( "subdomains" );
			for ( String subdomain : subdomains )
				section.createSection( subdomain );
		}

		ConfigurationSection section = yaml.getConfigurationSection( "subdomains" );
		subdomains = Lists.newArrayList();

		if ( section != null )
			for ( String subdomain : section.getKeys( false ) )
			{
				subdomains.add( subdomain );

				if ( !section.isConfigurationSection( subdomain ) )
					section.createSection( subdomain );

				SiteManager.getLogger().fine( String.format( "Initalized subdomain '%s' for site '%s'", subdomain, siteId ) );
			}
		 ...
		 */
		if ( publicDirectory( "~wisp" ).exists() )
	...
	protected void loadSsl()
	{
		loadSsl( HttpsManager.INSTANCE );
	}

	@Override
	public void loadSsl( HttpsManager https )
	{
		Validate.notNull( getDomain() );

		File ssl = siteDir( "ssl" );
...

		{
			...
			try
			{
				if ( sslCertFile != null && sslKeyFile != null )
				{
					...
					https.addMapping( "*." + getDomain(), sslCert, sslKey, yaml.getString( "site.sslSecret" ) );
				}
			}
		...
		{
			ConfigurationSection section = yaml.getConfigurationSection( "subdomains." + sub );
			...
			try
			{
				if ( sslCertFile != null && sslKeyFile != null )
				{
					...
					https.addMapping( sub + "." + getDomain(), sslCert, sslKey, section.getString( "sslSecret" ) );
				}
			}
			catch ( SSLException | FileNotFoundException e )
			{
	...
	private File publicDirectory( String dir )
	{
		return publicDirectory( dir, null );
	}

	private File publicDirectory( String dir, String subdomain )
	{
		if ( subdomain == null || subdomain.length() == 0 )
			subdomain = "root";

		File parent = new File( publicDirectory(), subdomain );
	...
	protected File siteDir( String dir )
	{
		return new File( siteDir, dir );
	}
	...
	public boolean subDomainExists( String subdomain )
	{
		...
		return subdomains.contains( subdomain.toLowerCase() );
	}
